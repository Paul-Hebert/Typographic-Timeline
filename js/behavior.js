var events = [	
				...
				[	
					["Pantograph",1603,"The first pantograph was constructed by Christoph Scheiner, who used the device to copy and scale drawings and diagrams. Later, the pantagraph was used to scale fonts to a variety of sizes and also to condense, extend, and slant letterforms.","Germany"]
				],
				[	
					...
					["Transitional",1700,"Type created in the middle of the eighteenth century when typestyles evolved from Old Style to Modern is known as Transitional. It is characterized by greater contrast between thick and thin strokes, more horizontal serifs in the lowercase, and less diagonal stress within rounded forms. Transitional forms are also ususally wider than old style characters.",'']
				],
				[	
				...
				[	
					["OCR-B",1968,"OCR-B, designed by the Swiss type designer Adrian Frutiger, is the European counterpart to OCR-A. It is designed to read by both machines and humans and it has a less technical look than OCR-A. Declared a world standard, it is widely used for UPC / EAN barcodes.","Switzerland"]
				],
				[	
				...
				[	
					...
					["Grunge",1990,"Grunge fonts are dirty, abstract, typefaces whose messey and edgy qualities represent the antithesis of the hygenic purity of the machine age. They were typically hand-scrawled scratchy marks that referenced urban graffiti. They represented the new found interest in the vernacular that had arisen alongside — and often in opposition to — the computer in the 1980s.",''],
					["Mashed",1990,"Adobe’s 1989 decision to end the encryption of its PostScript fonts coupled with the cut and paste features in software programs such as Fontographer, opend the door for hacking type. Mashed typefaces (or mash-ups) are a patchwork of pre-existing fonts cut and pasted from different sources. Experimental in nature, they were often somewhat crudely executed and intentionally unfinished and imperfect.",''],
...
function add_event_markers(){
	for(i=0; i < events.length; i++){
		...
		for(x=0; x < events[i].length; x++){
			...
			$('.event_details:last-of-type .event_buttons').append('<div class="button" id="button_' + i + '_' + x + '"  onclick="animate_event(' + i + ',' + x + ')">&nbsp;</div>');
...
function set_scroll_events(){
	$( window ).scroll(function() {
		...
		for(i=0; i < events.length; i++){
			if (current_year < events[i][0][1] + 1 && current_year > events[i][0][1] - 1){
...
function animate_event(container_num,event_num){
	...
	$('#button_' + container_num + '_' + event_num + '').css({'background':'#f00'});
